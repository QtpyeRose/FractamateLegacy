...
public class CodeRender {
    ...
    public static void main(String args[]) throws Exception {
        ...
        originRules.add(new Vector(0.5, 0.5, seed.length/Math.sqrt(2), seed.angle + (1 * 2 * Math.PI / 4)).getEndPoint());
...
//            }
            ...
            for (int i = 0; i < rulePoints.size(); i++) {
                if (i != rulePoints.size() - 1) {
                    StdDraw.textLeft(0.01, 0.9 - 0.025 * i, " " + rulePoints.get(i).copyShift(-0.5, -0.5) + ",");
                } else {
                    StdDraw.textLeft(0.01, 0.9 - 0.025 * i, " " + rulePoints.get(i).copyShift(-0.5, -0.5));
